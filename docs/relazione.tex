...
\author{Mirko Mariotti \\ \texttt{mirko@mirkomariotti.it}}
\date{Marzo 2026}
...
PeshMind è un sistema avanzato per la scoperta e l'analisi della topologia di rete che utilizza l'inferenza logica Prolog per ricostruire la topologia di una rete LAN basandosi esclusivamente sugli indirizzi MAC osservati dagli switch di rete. Il sistema combina un'applicazione CLI scritta in Go con un motore di ragionamento Prolog per identificare le connessioni di rete, gli switch interni/di accesso e i potenziali "ghost switch" (switch non gestiti) presenti nell'infrastruttura di rete.

\subsection{Motivazione}

In reti complesse di grandi dimensioni, la documentazione della topologia fisica spesso diventa obsoleta o incompleta. PeshMind affronta questo problema fornendo un sistema automatizzato che:

\begin{itemize}
    \item Ricostruisce la topologia di rete analizzando solo le tabelle MAC degli switch
    \item Identifica automaticamente switch non documentati ("ghost switch")
    ...
    \item Permette simulazioni per validare configurazioni di rete
\end{itemize}

\subsection{Obiettivi del Progetto}

Gli obiettivi principali di PeshMind sono:

\begin{enumerate}
    \item Automatizzare la scoperta della topologia di rete
    \item Ridurre l'errore umano nella documentazione di rete
    \item Fornire un sistema di interrogazione logica per analisi di rete
    \item Identificare anomalie e configurazioni non documentate
    \item Supportare la pianificazione e simulazione di modifiche alla rete
\end{enumerate}
...
L'applicazione CLI, sviluppata in Go utilizzando il framework Cobra, fornisce l'interfaccia utente per:

\begin{itemize}
    \item Gestione della configurazione degli switch
...
Il flusso di lavoro del sistema segue questi passaggi:

\begin{enumerate}
    \item \textbf{Configurazione}: Gli switch vengono configurati in \texttt{peshmind.json}
    \item \textbf{Acquisizione Dati}: Le tabelle MAC vengono scaricate tramite SSH
    \item \textbf{Conversione}: I dati vengono convertiti in fatti Prolog
    \item \textbf{Caricamento KB}: I fatti vengono caricati nella knowledge base
    \item \textbf{Inferenza}: Le regole Prolog deducono la topologia
    \item \textbf{Interrogazione}: L'utente può interrogare il sistema
    \item \textbf{Visualizzazione}: I risultati vengono presentati o visualizzati
\end{enumerate}
...
\subsection{Regole di Inferenza Prolog}

Il sistema utilizza diverse regole Prolog per inferire la topologia di rete:

\subsubsection{Predicato far/2}
...
\subsubsection{Predicato direct/2}
...
\subsubsection{Predicato directp/4}
...
\subsubsection{Predicato ghost/2}
...
\subsubsection{Predicato internalswitch/1}
...
\subsubsection{Predicato edgeswitch/1}
...
Il file \texttt{peshmind.json} definisce gli switch e i parametri di sistema:
...
Il linguaggio Go è stato scelto per l'applicazione CLI per:
\begin{itemize}
    \item Performance eccellente
    \item Compilazione statica (singolo binario)
    \item Eccellente supporto per concorrenza
    \item Ricco ecosistema di librerie
\end{itemize}

\subsection{SWI-Prolog}

SWI-Prolog fornisce il motore di inferenza con:
\begin{itemize}
    \item Potente sistema di inferenza logica
    \item Libreria Pengines per HTTP API
    \item Sintassi dichiarativa per regole complesse
    \item Eccellenti performance su knowledge base di grandi dimensioni
\end{itemize}

\subsection{Librerie Go Utilizzate}
...
\section{Esempi di Query Prolog}

Di seguito alcuni esempi di query utili:

\subsection{Trovare Tutti gli Switch}

\begin{lstlisting}
?- switch(X).
\end{lstlisting}

\subsection{Trovare Switch Direttamente Connessi}

\begin{lstlisting}
?- directn('switch-01', Y).
\end{lstlisting}

\subsection{Identificare Porte di Connessione}

\begin{lstlisting}
?- directpn('switch-01', 'switch-02', PX, PY).
\end{lstlisting}

\subsection{Trovare Tutti i Ghost Switch}

\begin{lstlisting}
?- ghostn(X, Y).
\end{lstlisting}

\section{Script Bash per Automazione}

\subsection{Script di Update Periodico}

\begin{lstlisting}
#!/bin/bash
# update-network.sh
# Aggiorna automaticamente la topologia di rete

PESHMIND="/usr/local/bin/peshmind"
KBPOOL="/var/lib/peshmind/kbpool"
LOGFILE="/var/log/peshmind/update.log"

echo "$(date): Starting network update" >> $LOGFILE

# Update all switches
$PESHMIND update --switch all >> $LOGFILE 2>&1

if [ $? -eq 0 ]; then
    echo "$(date): Update completed successfully" >> $LOGFILE
    
    # Generate topology visualization
    $PESHMIND dot > /var/www/html/topology.dot
    dot -Tsvg /var/www/html/topology.dot \
        -o /var/www/html/topology.svg
else
    echo "$(date): Update failed" >> $LOGFILE
    exit 1
fi
\end{lstlisting}
...
File \texttt{peshmind.json} per rete a 4 switch:

\begin{lstlisting}[basicstyle=\ttfamily\footnotesize]
{
  "debug": false,
  "endpoint": "http://localhost:3030/pengine",
  "sim_generate_percentage": 100,
  "username_default": "admin",
  "password_default": "ask",
  "switches": {
    "core-sw-01": {
      "name": "core-sw-01",
      "mac": "e4115b740100",
      "description": "Switch Core Edificio A",
      "ip": "10.0.0.1",
      "username": "admin",
      "password": "ask",
      "model": "HP-Aruba",
    ...
    "dist-sw-02": {
      "name": "dist-sw-02",
      "mac": "002347bb73c0",
      "description": "Switch Distribuzione Piano 2",
      "ip": "10.0.0.2",
      "username": "admin",
      "password": "ask",
      "model": "HP-Aruba",
    ...
    "access-sw-03": {
      "name": "access-sw-03",
      "mac": "0011856d4640",
      "description": "Switch Accesso Laboratorio",
      "ip": "10.0.3.5",
      "username": "admin",
      "password": "ask",
    ...
    "access-sw-04": {
      "name": "access-sw-04",
      "mac": "e0071ba2d8a0",
      "description": "Switch Accesso Uffici",
      "ip": "10.0.3.30",
      "username": "admin",
      "password": "ask",
      "model": "HP-Aruba",
